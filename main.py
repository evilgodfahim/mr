...
PROMPT = """Classify news article titles. Default: NOISE. Bar: EXTREME. Output only: {{"signal": [0-based indices]}}. Valid JSON, no markdown, no explanation.

INSTANT NOISE — reject immediately:
Sports · entertainment · celebrity · lifestyle · tribute · commemorative · partisan praise · isolated local incident

SIGNAL — Bangladesh directly involved:
• National scale: economy, trade, exports, remittances, inflation, banking, reserves, currency, government policy, public system failure, infrastructure, disaster, health emergency — national reach = SIGNAL
• Foreign affairs: bilateral talks or disputes, international pressure or sanctions on BD, foreign aid or loans, cross-border issues (water, trade, security, migration), BD at international forums
• Editorial: names a concrete national domain → SIGNAL. Vague sentiment, party strategy, or praise → NOISE.

SIGNAL — Bangladesh not involved:
• Multinational bodies acting collectively: UN agencies, NATO, IMF, World Bank, WTO, G7/G20, BRICS, IAEA, ICC, ICJ — their resolutions and interventions are SIGNAL
• Multi-country: wars, conflicts, cross-border crises, multilateral treaties, regional instability, international sanctions
• Single-country with global consequence: moves global energy, financial systems, or trade architecture; or shifts strategic power (nuclear decisions, major arms deals, upstream water control affecting downstream nations, military repositioning, treaty withdrawal)
• All other single-country internal affairs → NOISE

DEDUP: among selected signals, remove near-duplicates (same story, rephrased headline). Keep only the lowest index per group.
...
def fetch_all_feeds():
    ...
    for url in FEED_URLS:
        ...
        for e in feed.entries:
            ...
            elif e.get("content") and isinstance(
                e.get("content"),
                list
            ):
                desc = "\n".join([
...
def send_to_mistral(articles):
    ...
    try:
        ...
        return extract_json_object(text).get(
            "signal",
            []
        )
...
def normalize_title_for_dedup(title):
    ...
    tokens = [
        ...
        if t not in {
            ...
            "from", "by", "with", "and", "or", "at", "as",
            "is", "are", "was", "were", "be", "been", "being",
            "after", "before", "over", "under", "amid", "into",
            "about", "news", "report", "reports", "says", "said"
        }
    ]
...
def deduplicate_signal_articles(articles):
    ...
    if dropped:
        print(
            ...
            f"near-duplicate signal title(s)."
        )
...
def main():
    ...
    excluded_articles = [
        ...
        if i not in set(mistral_indices)
    ]
    ...
    signal_articles = deduplicate_signal_articles(
        signal_articles
    )
