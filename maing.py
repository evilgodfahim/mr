...
PROMPT = """You are a geopolitics classification engine. Classify each numbered headline as SIGNAL or NOISE. Bar is HIGH — when in doubt, NOISE.

GEOPOLITICS = interplay of power between states, alliances, and international bodies across military, diplomatic, and strategic dimensions.

INSTANT NOISE — stop here if:
Sports · entertainment · celebrity · lifestyle · human interest · domestic economic data with no foreign-policy angle · internal party politics · isolated local incidents · corporate news · natural disasters unless used as a geopolitical instrument

SIGNAL if any one applies:
A. INTER-STATE DIPLOMACY — summits, treaties, normalisation/rupture of ties where 2+ states are direct parties
B. ARMED CONFLICT — wars, airstrikes, invasions, territorial changes, ceasefires, arms deals, troop deployments, nuclear/missile events
C. COERCIVE INSTRUMENTS — sanctions regimes, state-attributed cyberops, blockades, trade warfare as geopolitical leverage (not routine trade policy)
D. MULTILATERAL SECURITY BODIES — UNSC resolutions/vetoes, NATO decisions, IAEA nuclear findings, ICC/ICJ on state conduct, G7/G20 responding to geopolitical crises (routine IMF/World Bank loans → NOISE)
E. GREAT-POWER COMPETITION — US–China, US–Russia, China–India rivalries; alliance realignments, new security pacts, treaty withdrawals, strategic infrastructure (ports, pipelines, military bases)
F. TERRITORIAL/MARITIME DISPUTES — border conflicts, island/resource-zone claims, water control used as interstate leverage
G. INTELLIGENCE & STRATEGIC SIGNALING — state-attributed espionage, significant intelligence disclosures, credible threats of military action
H. BANGLADESH FOREIGN AFFAIRS — bilateral talks or disputes (India, China, US, Myanmar), international pressure or sanctions on BD, cross-border issues, BD as direct party in international forums → SIGNAL. BD domestic politics, internal economics alone → NOISE.
I. GEOPOLITICAL ANALYSIS — SIGNAL only if a named state-level event, conflict, or power dynamic is the explicit subject. Vague civilisational commentary or partisan opinion with no named states → NOISE.

DEDUPLICATION — among your SIGNAL set, keep only the lowest index for near-duplicate stories covering the same event or rephrased from the same headline.
...
def load_processed_articles():
    ...
    id_ts   = {
        k: v
        for k, v in data.get("id_timestamps", {}).items()
        if v >= cutoff
    }

    link_ts = {
        k: v
        for k, v in data.get("link_timestamps", {}).items()
        if v >= cutoff
    }
...
def get_new_articles(all_articles, processed_data):
    processed_ids = set(
        processed_data.get("article_ids", [])
    )

    processed_links = set(
        processed_data.get("article_links", [])
    )
...
def dedup_by_link(articles):
    ...
    if dropped:
        print(
            f"Link dedup: removed {dropped} "
            f"duplicate link(s) before API call."
        )
...
def main():
    ...
    generate_xml_feed(
        ...
        feed_description=(
            "AI-curated signal: geopolitical "
            "and Bangladesh foreign-affairs news"
        ),
    )
